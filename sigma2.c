...
/*														*
   ...
   Obtiene la secuencia de un fichero de texto en formato
   PEARSON/FASTA.
*														*/
{
    register int i;
...
*																	*/
{
    ...
    if ((secuencia[0] == 'g') || (secuencia[0] == 'a')) {	/* x-R-?-x */
	if ((secuencia[1] == 't') || (secuencia[1] == 'c')) {	/* x-R-Y-x */
	    enlace[0].sigma1 = -2;
	    enlace[1].sigma1 = 1;
	}
	/* else x-R-R-x sigma1 = 0 */
    } else {			/* x-Y-?-x */
	if ((secuencia[1] == 'g') || (secuencia[1] == 'a')) {	/* x-Y-R-x */
	    enlace[0].sigma1 = -4;
	    enlace[1].sigma1 = 2;
	}
	...
	if ((secuencia[i] == 'g') || (secuencia[i] == 'a')) {	/* x-R-?-x */
	    if ((secuencia[i + 1] == 't') || (secuencia[i + 1] == 'c')) {	/* x-R-Y-x */
		enlace[i - 1].sigma1 += 1;
		enlace[i].sigma1 -= 2;
		enlace[i + 1].sigma1 += 1;
	    }
	    ...
	    if ((secuencia[i + 1] == 'g') || (secuencia[i + 1] == 'a')) {	/* x-Y-R-x */
		enlace[i - 1].sigma1 += 2;
		enlace[i].sigma1 -= 4;
		enlace[i + 1].sigma1 += 2;
	    }
...
/*																	*
   ...
   azúcar fosfato hacya las Py) para la secuencia en estudio.
...
*																	*/
{
	...
	if ((secuencia[i] == 'a') || (secuencia[i] == 'g')) {	/* Si la base en curso es R */
	    if ((secuencia[i + 1] == 't') || (secuencia[i + 1] == 'c')) {	/* R-Y */
		enlace[i].sigma3 += 1;
		enlace[i + 1].sigma3 -= 1;
	    }
	    /* else R-R : quedan como estaban */
	} else {		/* la base en curso es Y */
	    if ((secuencia[i + 1] == 'a') || (secuencia[i + 1] == 'g')) {	/* Y-R */
		enlace[i].sigma3 -= 2;
		enlace[i + 1].sigma3 += 2;
	    }
...
*																	*/
{
    ...
    move(x1, y1);
    label
	("SIGMA 4: cómputo del ángulo de propulsión para cada par de bases");
...
*																*/
{
	...
	ynueva =
	    basev -
	    ((enlace[inicio + i].sigma1 * enlace[inicio].sigma1 +
	      16.0) * incv);
	x1 = floor(xvieja);
	y1 = floor(yvieja);
	x2 = floor(xnueva);
