...
TEMP=`getopt -o hvribBMAmlnS:R: \
     --long help,verbose,roche454,illumina,bwa,bowtie2,maq,a5,mummer3,last,nucmer,sequences:,reference: \
     -n "$0" -- "$@"`
...
# =============
if [ ! -d "$SEQDIR" ] ; then
    echo "Original sequences should be in a folder/directory named '$SEQDIR'"
    exit
fi
...
#    if [ -e ../$SEQDIR/*_R1_001.fastq ] ; then
#    	ln -s ../$SEQDIR/*_R1_001.fastq .
#    elif [ -e ../$SEQDIR/*_R1_001.fastq.gz ] ; then
#        cp ../$SEQDIR/*_R1_001.fastq.gz .
...
#    if [ -e ../$SEQDIR/*_R2_001.fastq ] ; then
#    	ln -s ../$SEQDIR/*_R2_001.fastq .
#    elif [ -e ../$SEQDIR/*_R2_001.fastq.gz ] ; then
#        cp ../$SEQDIR/*_R2_001.fastq.gz .
#    	gunzip ../$SEQDIR/*_R2_001.fastq.gz
#    else
...
if [ ! "$(ls -A *_R1_001.fastq)" ] ; then
    if [ "$(ls -A ../$SEQDIR/*_R1_001.fastq)" ] ; then
    	ln -s ../$SEQDIR/*_R1_001.fastq .
    elif [ "$(ls -A ../$SEQDIR/*_R1_001.fastq.gz)" ] ; then
        ln -s ../$SEQDIR/*_R1_001.fastq.gz .
    	gunzip *_R1_001.fastq.gz
    else
        echo "Error: no R1 file found"
        exit
...
if [ ! "$(ls -A *_R2_001.fastq)" ] ; then
    if [ "$(ls -A ../$SEQDIR/*_R2_001.fastq)" ] ; then
    	ln -s ../$SEQDIR/*_R2_001.fastq .
    elif [ "$(ls -A ../$SEQDIR/*_R2_001.fastq.gz)" ] ; then
        ln -s ../$SEQDIR/*_R2_001.fastq.gz .
...
if [ "$SEQUENCE_IS_454" = "yes" ] ; then
    ...
    if [ ! -e reference.fna ] ; then
	    ...
	    ln -s ../refs/reference.fna .
            ...
            echo "We need a reference sequence in refs/reference.fna for the"
	    echo "reference assemblers and ulterior comparisons"
	    exit
        fi
    fi
    if [ ! -e R1.fastq ] ; then
...
if [ "$SEQUENCE_IS_ILLUMINA" = "yes" ] ; then
    ...
    if [ ! -e reference.fna ] ; then
	    ...
	    ln -s ../refs/reference.fna .
            ...
            echo "We need a reference sequence in refs/reference.fna"
